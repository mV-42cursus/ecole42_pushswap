...
/*   Updated: 2023/06/21 02:47:23 by marvin           ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
char	**ft_split_ps(char **av, char c)
{
	...
	word_cnt = ft_count_words_ps(av, c);
	...
	while (++rotation < word_cnt)
	{
		while (*av[1] == c || *av[1] == '\0')
			av[1]++;
		word_len = ft_count_wordlen_ps(av[1], c);
		res[rotation] = (char *)malloc(sizeof(char) * (word_len + 1));
		if (res[rotation] == NULL)
			return (ft_memory_manager_ps(res, rotation));
		ft_word_cpy_ps(&res, av[1], word_len, rotation);
		av[1] = av[1] + word_len;
	}
	res[rotation] = NULL;
	return (res);
